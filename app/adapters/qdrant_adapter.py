...
class QdrantAdapter:
    ...
    async def search(self,query:list[float],limit:int,filters: MetaData | None = None)-> list[SearchResult]:
        qdrant_filter = None
        if filters and filters.match_conditions:
        ...
        try:
            raw_results = await self.client.search(collection_name=self.collection_name,query_filter=qdrant_filter,query_vector=query,limit=limit)
