...
from app.tests.unit.fake_vectorStore import FakeVectorStore
...
def test_idempotent_ingestion(tmp_path):
    ...
    assert len(vector_store.store) == 1  # Assuming the document is chunked into one batch
    ...
    assert len(vector_store.store) == 1  # The number of keys in the store should remain the same
